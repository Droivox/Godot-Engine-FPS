...
func _explode_others() -> void:
	for bodie in $explosion.get_overlapping_bodies():
		if bodie.has_method("_damage") and bodie != self and "durability" in bodie and bodie.durability > 0:
				var explosion_distance = (5.0 * bodie.global_transform.origin.distance_to(global_transform.origin))
				bodie._damage(300.0 - explosion_distance)
