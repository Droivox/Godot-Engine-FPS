class weapon:
	...
	var look_up = Vector3(1, 1, 0)

	var rot_to = -deg2rad(40)
	...
	func _shoot(delta: float) -> void:
		...
		if bullets > 0:
			...
			if animc != "Shoot" and animc != "Reload" and animc != "Draw" and animc != "Hide":
				...
				# recoil
				owner.camera.rotation.x = lerp(owner.camera.rotation.x, rand_range(1.0, 2.0), delta)
				owner.camera.rotation.y = lerp(owner.camera.rotation.y, rand_range(-1.0, 1.0), delta)
				...
				if ray.is_colliding():
					...
					decal.look_at(ray.get_collision_point() + ray.get_collision_normal(), look_up)
		else:
			# Play out sound
	...
	func _zoom(input: int, delta: float) -> void:
		var camera = owner.camera

		if input and animc != "Reload" and animc != "Hide" and animc != "Draw":
			camera.fov = lerp(camera.fov, 40, lerp_speed * delta)
			...
			camera.fov = lerp(camera.fov, 70, lerp_speed * delta)
