...
func _weapon(delta: float) -> void:
	...
	arsenal.values()[current]._sprint(character.input["sprint"] or character.input["jump"], delta)

	if not character.input["sprint"] or not character.direction:
		if input["shoot"]:
			arsenal.values()[current]._shoot(delta)

		arsenal.values()[current]._zoom(input["zoom"], delta)
